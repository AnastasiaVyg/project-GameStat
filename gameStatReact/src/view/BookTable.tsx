...
import {loadGames, loadPlayers, loadBooks, addBook, updateBook, deleteBook, loadComments} from "../store/Actions";
...
export function BookTable() {
    ...
    const isLoadedAuthors = useSelector((state: AppState) => state.isLoadedPlayers)
    const isLoadedGenres = useSelector((state: AppState) => state.isLoadedGames)
    const books = useSelector((state: AppState) => {return state.books})
    const genres = useSelector((state: AppState) => {return state.games})
    const authors = useSelector((state: AppState) => {return state.players})
...
function AuthorDetailsPanel(props: BookDetailsProps) {
    ...
    if (props.book == Book.empty) {
        ...
        return (<Container maxWidth="sm">
            ...
            <div>
                <Typography variant="h4" color="textPrimary">{book.author.name}</Typography>
            </div>
            <div>
